...
//AppFormBean appFormBean=new AppFormBean();
		...
		//for (Application app : appDtls) {
			...
			//	applicationBean.setCeStatus("Application submitted; verification and inspection pending1.");
				...
				applicationBean.setCeStatus("Under by EE. Initial processing cost Rs "+appDtls.getInitialPayment()+"  send it to Your registered mobile and email.Please pay this url :  http://localhost:8080/WaterNew/initialPayment.do");
				//applicationBean.setRemarks(appDtls.getRemarks().toString());
			}
			else{
				applicationBean.setCeStatus("payment successful");
...
///
	...
	public String listAfterInspection() {
		...
		List<Application> appDtls = dashboardDao.listAfterInspection();
		List<ApplicationBean> applicationBeanList = new ArrayList<ApplicationBean>();
		for (Application app : appDtls) {
			MasterZone mastr=new MasterZone();
			mastr.setZoneDesc("other");
	...
	public String listBeforeInspection() {
		...
		List<Application> appDtls = dashboardDao.listBeforeInspection();
		List<ApplicationBean> applicationBeanList = new ArrayList<ApplicationBean>();
		for (Application app : appDtls) {
			MasterZone mastr=new MasterZone();
			mastr.setZoneDesc("other");
	...
	@Produces(MediaType.APPLICATION_JSON)
	public String SendInspectionDate(ApplicationBean applicationBean) {
