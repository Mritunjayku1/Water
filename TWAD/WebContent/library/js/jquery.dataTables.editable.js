...
(function($) {
	...
	$.fn.makeEditable = function(options) {
		...
		// By default it is assumed that id is placed as an id attribute of <tr>
		// that that surround the cell (<td> tag). E.g.:
		// <tr id="17">
		...
		// It is assumed that id is placed as an id attribute of <tr> that that
		// surround the cell (<td> tag). E.g.:
		// <tr id="17">
		// <td>...</td><td>...</td><td>...</td><td>...</td>
		// </tr>
		...
		// /@param action - action that was executed when error occured e.g.
		...
		function _fnApplyEditable(aoNodes) {
			...
			var oDefaultEditableSettings = {
				...
				"callback" : function(sValue, settings) {
					...
					properties.fnOnEdited(status, sOldValue,
							sNewCellDisplayValue, aPos[0], aPos[1], aPos[2]);
				...
				"onsubmit" : function(settings, original) {
					...
					if (input.length == 1) {
						...
						if (oEditElement.nodeName.toLowerCase() == "select"
								|| oEditElement.tagName.toLowerCase() == "select") {
							sNewCellDisplayValue = $("option:selected",
									oEditElement).text(); // For select list
							// use selected text
							// instead of value
							// for displaying in
							// table
				...
				"onerror" : function() {
					...
					properties.fnShowError(
							"Cell cannot be updated(Server error)", "update");
			...
			if (properties.aoColumns != null) {
				for ( var i = 0; i < properties.aoColumns.length; i++) {
					if (properties.aoColumns[i] != null) {
						...
						oColumnSettings = $.extend({},
								oDefaultEditableSettings,
								properties.aoColumns[i]);
				...
				cells = $('td:not(.' + properties.sReadOnlyCellClass + ')',
						aoNodes);
		...
		function _fnOnRowAdding(event) {
			...
			// alert("inside _fnOnRowAdding"+properties.fnOnAdding());
			if (properties.fnOnAdding()) {
				if (oAddNewRowForm.valid()) {
					...
					if (properties.sAddURL == "AddDeviceEnrollDtls.action") {
						// alert("fnCheckBoxStatusForEmail()
						// :"+fnCheckBoxStatusForEmail());
						if (fnOwnerShipRadioButtonStatus()) {
							if (fnCheckBoxStatusForEmail()) {
						...
						$.ajax({
							...
							error : function(response) {
								properties.fnEndProcessingMode();
								properties.fnShowError(response.responseText,
										"add");
								properties.fnOnAdded("failure");
							}
						});
		...
		// /Event handler called when a new row is added and response is
										...
										properties
												.fnShowError(
														"In the add form is placed input element with the name '"
																+ $(this).attr(
																		"name")
																+ "' with the 'rel' attribute that must be less than a column count - "
																+ iColumnCount,
														"add");
									} else {
										if (this.nodeName.toLowerCase() == "select"
												|| this.tagName.toLowerCase() == "select") {
											sCellValue = $("option:selected",
													this).text();
		...
		function fnOnCloseForm(event, ui) {
			oAddNewRowForm.dialog(properties.oAddNewRowFormOptions);

			// Clear the validation messages and reset form
			$(oAddNewRowForm).validate().resetForm(); // Clears the validation
			// errors

			// --------------------Added by chinnapparaj for Assign sm to branch
			// form===========

			$("#ddlstSMNameFORBRANCH").addClass('required');
			$("#smlabelFORBRANCH").css("display", "none");
			$("#ddlstSMNameFORBRANCH").css("display", "block");

			// end of chinnapparaj

			if (oAddNewRowForm.dialog("isOpen")) {
				$(oAddNewRowForm)[0].reset();
		...
		function _fnDeleteRow(id, sDeleteURL) {
			...
			properties.fnStartProcessingMode();
			var data = $.extend(properties.oDeleteParameters, {
				"id" : id
			});
			$.ajax({
				'url' : sURL,
		...
		function _fnOnRowDelete(event) {
			...
			var id = fnGetCellID($(
					'tr.' + properties.sSelectedRowClass + ' td', oTable)[0]);
			if (properties.fnOnDeleting($('tr.' + properties.sSelectedRowClass,
					oTable), id, _fnDeleteRow)) {
				_fnDeleteRow(id);
			}
		}
		...
		function _fnOnRowDeleted(response) {
			...
			if ($.trim(response) == "ok" || $.trim(response) == "") {
		...
		/*
		 * Function called before row is deleted @param tr JQuery wrapped around
		 * the TR tag that will be deleted @param id id of the record that wil
		 * be deleted @return true if plugin should continue with deleting row,
		 * false will abort delete.
		 */
		...
		/*
		 * Function called after delete action @param result string "success" if
		 * row is actually deleted "failure" if delete failed @return void
		 */
		...
		function fnOnEdited(result, sOldValue, sNewValue, iRowIndex,
				iColumnIndex, iRealColumnIndex) {
		...
		function fnOnAdding() {
			...
			fnCheckSessionOnAdd();
			var urlArg = $("#errorSessionText").val();

			if (urlArg == 'error') {
				window.location = 'Login.action';
			} else if (urlArg == 'success') {
				return true;
			}
			return true;
		}
				...
				.each(function() {
					...
					if (oAddNewRowForm.length != 0) {
						if (properties.oAddNewRowFormOptions != null) {
							...
							properties.oAddNewRowFormOptions = {
								autoOpen : false
							};
						}
						oAddNewRowForm.dialog(properties.oAddNewRowFormOptions);
						...
						if (oAddNewRowButton.length != 0) {
							oAddNewRowButton.click(function() {
								...
								$("#formAddNewRow input#CRUD").val("ADD");
							...
							if ($(properties.sAddDeleteToolbarSelector).length == 0) {
								...
								oAddNewRowButton = null; // It will be
						...
						if (oConfirmRowAddingButton.length == 0) {
							...
							aAddNewRowFormButtons
									.push(properties.oAddNewRowOkButtonOptions);
						} else {
						...
						if (oCancelRowAddingButton.length == 0) {
							...
							aAddNewRowFormButtons
									.push(properties.oAddNewRowCancelButtonOptions);
						} else {
							oCancelRowAddingButton.click(_fnOnCancelRowAdding);
						}
						// if the array contains elements, add them to the
						// dialog
						if (aAddNewRowFormButtons.length > 0) {
							oAddNewRowForm.dialog('option', 'buttons',
									aAddNewRowFormButtons);
						}
						...
						// oCancelRowAddingButton = $("#" +
						// properties.sAddNewRowCancelButtonId, oAddNewRowForm);
						oConfirmRowAddingButton = $("#"
								+ properties.sAddNewRowOkButtonId);
						oCancelRowAddingButton = $("#"
								+ properties.sAddNewRowCancelButtonId);
						...
						oAddNewRowForm
								.bind(
										"dialogopen",
										function(event, ui) {
											if ($("#formAddNewRow input#CRUD")
													.val() == "ADD") {
												DiffFormMode("ADD");
											} else if ($(
													"#formAddNewRow input#CRUD")
													.val() == "EDIT") {
												DiffFormMode("EDIT");
											}
										});
					...
					// If an add and delete buttons does not exists but
					// Add-delete toolbar is specificed
					...
					if (oAddDeleteToolbar.length != 0) {
						if (oAddNewRowButton == null
								&& properties.sAddNewRowButtonId != ""
								&& oAddNewRowForm != null) {
							oAddDeleteToolbar.append("<button id='"
									+ properties.sAddNewRowButtonId
							...
							oAddNewRowButton
									.click(function() {
										$("#formAddNewRow input#CRUD").val(
												"ADD");

										// ========changed by raja on 29/07/2013
										$('#formAddNewRow')
												.each(
														function() {
							...
							oAddNewRowForm.bind("dialogclose", function(event,
									ui) {
								fnOnCloseForm(event, ui);
							});
						}
						/*
					...
					// If delete button exists disable it until some row is
					// selected
					if (oDeleteRowButton != null) {
						if (properties.oDeleteRowButtonOptions != null) {
							oDeleteRowButton
									.button(properties.oDeleteRowButtonOptions);
						}
						_fnDisableDeleteButton();
					}
					...
					if (oAddNewRowButton != null) {
						if (properties.oAddNewRowButtonOptions != null) {
							oAddNewRowButton
									.button(properties.oAddNewRowButtonOptions);
						}
					}

					// If form ok button exists convert it to the JQuery-ui
					// button
					if (oConfirmRowAddingButton != null) {
						if (properties.oAddNewRowOkButtonOptions != null) {
							oConfirmRowAddingButton
									.button(properties.oAddNewRowOkButtonOptions);
						}
					}

					// If form cancel button exists convert it to the JQuery-ui
					// button
					if (oCancelRowAddingButton != null) {
						if (properties.oAddNewRowCancelButtonOptions != null) {
							oCancelRowAddingButton
									.button(properties.oAddNewRowCancelButtonOptions);
						}
					}

					// Add handler to the inline delete buttons
					$(".table-action-deletelink", oTable).live(
							"click",
							function(e) {
								...
								if (properties.fnOnDeleting(oTD, id,
										_fnDeleteRow)) {
...
function fnCheckSessionOnAdd() {
	...
	$.ajax({
		...
		success : function(response) {
			$("#errorSessionText").val(response);
		},
		error : function(eerr, textStatus) {
			alert("Internal Error :" + eerr + " - " + textStatus);
		}
	});
}
